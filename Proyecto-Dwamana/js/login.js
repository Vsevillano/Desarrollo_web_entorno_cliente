{
	...
	*/
	let validarInput = function(event,span) {
	...
	*/
	let validarPassword = function(event,span) {
	...
	*/
	let validarCampos = function(event) {
		...
		$.getJSON('js/usuarios.json',function(data) {	
			$.each(data['usuarios'],function(key,value){
				if((value['usuario'] == $userInput.val()) && value['password'] == $passInput.val())
					loggeado = true;
				loggeado ? window.location ="index.html" : $passwdSpan.css("color","red").html("Usuario o contraseña no encontrados");	
